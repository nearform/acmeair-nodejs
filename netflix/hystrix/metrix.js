...
var rollingStatisticalWindowInMilliseconds =
  settings.hystrix.rollingStatisticalWindowInMilliseconds || 5000;
...
var getBucket = function(ts) {
  return Math.ceil(ts / bucketSizeInMilliseconds);
};
...
Metrix.prototype.showData = function() {
  console.log(this.name, this.group, this.ts, JSON.stringify(this.data)),
    this.error,
    this.circuitOpen,
    this.timeout;
};
...
Metrix.prototype.getStatistics = function(callback /* error, result*/) {
  ...
  if (this.statistics && this.statistics.bucket == bucket) {
    // the last calculated one is the current one
    callback(null, this.statistics);
    return;
  }
  ...
  this.statistics = {
    ...
    statistics: {
      ...
      error: totalError,
      errorRate: errorRate,
      totalCircuitOpen: totalCircuitOpen,
      totalTimeout: totalTimeout,
      mean: stats.mean(totalList),
      stddev: stats.stdev(totalList),
      percentile: {
        '0': stats.percentile(totalList, 0),
        '25': stats.percentile(totalList, 0.25),
        '50': stats.percentile(totalList, 0.5),
        '75': stats.percentile(totalList, 0.75),
        '90': stats.percentile(totalList, 0.9),
        '95': stats.percentile(totalList, 0.95),
        '99': stats.percentile(totalList, 0.99) || 0,
        '99.5': stats.percentile(totalList, 0.995) || 0,
        '100': stats.percentile(totalList, 1) || 0,
      },
    },
  };
...
Metrix.prototype.push = function(item, isError, isCircuitOpen, isTimeout) {
  ...
  var index = this.ts.length - 1;
  if (index < 0 || this.ts[index] != bucket) {
    // the last item is not the current bucket
    index++;
    this.ts.push(bucket);
    this.data.push([]);
    this.error.push(0);
...
function housekeeping(bucket, metrix) {
  ...
  if (total > allowedBucketLength) {
    for (var i = 0; i < total; i++) {
      if (metrix.ts[i] >= bucket - bucketLength) {
        // fall into the rolling window
        if (i > 0) {
          // remove the [0 ,i-1] item
