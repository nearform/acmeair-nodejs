...
var getCommand = function(name, theWork, callback /*error, command*/) {
  if (!AllCommands[name]) {
    ...
    var maxFailures =
      settings.hystrix.circuitBreaker.maxFailures || defaultOption.MAX_FAILURES;
    var callTimeout =
      settings.hystrix.circuitBreaker.callTimeoutInMilliseconds ||
      defaultOption.CALL_TIMEOUT_MS;
    var resetTimeout =
      settings.hystrix.circuitBreaker.resetTimeoutInMilliseconds ||
      defaultOption.RESET_TIMEOUT_MS;

    AllCommands[name] = new Command(
      name,
      theWork,
      maxFailures,
      callTimeout,
...
var Command = function(name, theWork, maxFailures, callTimeout, resetTimeout) {
  ...
  this.theWork = CircuitBreaker.new_circuit_breaker(
    theWork,
    maxFailures,
    callTimeout,
    resetTimeout
...
Command.prototype.execute = function() {
  ...
  arguments[callbackIndex] = function(error, result) {
    ...
    cmd.isCircuitOpen =
      error == null
        ? false
        : error instanceof CircuitBreaker.CircuitBreakerError;
    var isTimeout =
      error == null ? false : error instanceof CircuitBreaker.TimeoutError;
    Metrix.getInstance(cmd.name).push(
      responseTime,
      error != null,
      cmd.isCircuitOpen,
      isTimeout
