...
module.exports = function(dataAccess, fastifyService, settings, authService) {
  ...
  (flightDataCacheTTL =
    ...
    (flightSegmentCache = tllLruCache({
      maxLength: settings.flightDataCacheMaxSize,
    }));
  ...
  module.countSessions = async (req, reply) => {
    try {
      const count = await service.countItems(
        dataaccess.dbNames.customerSessionName
      );
      reply.send(count.toString());
    } catch (e) {
      reply.send('-1');
  ...
  module.countAirports = async (req, reply) => {
    try {
      const count = await service.countItems(
        dataaccess.dbNames.airportCodeMappingName
      );
      reply.send(count.toString());
    } catch (e) {
      reply.send('-1');
  ...
  module.queryFlights = async (req, reply) => {
    const getFlightByAirportsAndDepartureDate = async function(
      fromAirport,
      toAirport,
      flightDate
    ) {
      ...
      try {
        ...
        try {
          ...
          if (settings.useFlightDataRelatedCaching) {
            ...
            'about to populate the cache with flights key = ' +
              cacheKey +
              ' with value of ' +
              JSON.stringify(cacheValue);
            flightCache.set(cacheKey, cacheValue, flightDataCacheTTL);
            'after cache populate with key = ' +
              cacheKey +
              ', flightCacheSize = ' +
              flightCache.size();
          }
    ...
    const getFlightSegmentByOriginPortAndDestPort = async function(
      ...
      if (settings.useFlightDataRelatedCaching) {
        ...
        'cache miss - flightsegment search, key = ' +
          fromAirport +
          toAirport +
      ...
      try {
        ...
        if (settings.useFlightDataRelatedCaching) {
          'about to populate the cache with flightsegment key = ' +
            fromAirport +
            toAirport +
            ' with value of ' +
            JSON.stringify(segment);
          flightSegmentCache.set(
    ...
    const fromAirport = req.body.fromAirport,
      ...
      fromDate = new Date(
        fromDateWeb.getFullYear(),
        fromDateWeb.getMonth(),
        fromDateWeb.getDate()
    ...
    if (!oneWay) {
      returnDate = new Date(
        returnDateWeb.getFullYear(),
        returnDateWeb.getMonth(),
        returnDateWeb.getDate()
      ); // convert date to local timezone
    }
    try {
      const res = await getFlightByAirportsAndDepartureDate(
        fromAirport,
        toAirport,
        fromDate
      );
      ...
      if (!oneWay) {
        const result = await getFlightByAirportsAndDepartureDate(
          toAirport,
          fromAirport,
          returnDate
        );
  ...
  module.bookFlights = async (req, reply) => {
    ...
    if (!oneWay) {
      ...
      bookingInfo = {
        oneWay: false,
        returnBookingId: retBookingId,
        departBookingId: toBookingId,
      };
    } else {
      bookingInfo = { oneWay: true, departBookingId: toBookingId };
    }
    reply.header('Cache-Control', 'no-cache').send(bookingInfo);
  };
  ...
  const getBookingsByUser = async username => {
    return await service.findBy(dataaccess.dbNames.bookingName, {
      customerId: username,
    });
  };
  ...
  const cancelBooking = async (bookingid, userid) => {
    return await service.remove(dataaccess.dbNames.bookingName, {
      _id: bookingid,
      customerId: userid,
    });
  };
  ...
  const getCustomer = async username => {
    const result = await service.findOne(
      dataaccess.dbNames.customerName,
      username
    );
    return result;
  };
