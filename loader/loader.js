...
module.exports = function(loadUtil, settings) {
  ...
  async function insertAirportCodeMapping(airportCodeMapping) {
    ...
    await loadUtil.insertOne(
      loadUtil.dbNames.airportCodeMappingName,
      airportCodeMapping
    );
  }
  ...
  module.startLoadDatabase = async function startLoadDatabase(req, reply) {
    ...
    createFlightRelatedData(function() {
      ...
      logger.info(
        'number of airportCodeMappings = ' + airportCodeMappings.length
      );
  ...
  module.getNumConfiguredCustomers = function(req, reply) {
    reply.type('text/plain').send(loaderSettings.MAX_CUSTOMERS.toString());
  };
  ...
  var airportCodeMappingQueue = async.queue(
  ...
  var flightSegmentsQueue = async.queue(
    insertFlightSegment,
    DATABASE_PARALLELISM
  );
    ...
    csv()
      ...
      .on('end', function(count) {
        ...
        for (var ii = 0; ii < rows[0].length; ii++) {
          var airportCodeMapping = cloneObjectThroughSerialization(
            airportCodeMappingTemplate
          );
        ...
        for (var ii = 2; ii < rows.length; ii++) {
          ...
          for (var jj = 2; jj < rows[ii].length; jj++) {
            ...
            mileage = rows[ii][jj];
            if (mileage != 'NA') {
              var flightSegment = cloneObjectThroughSerialization(
              ...
              for (
                var kk = 0;
                kk < loaderSettings.MAX_DAYS_TO_SCHEDULE_FLIGHTS;
                kk++
              ) {
                for (
                  var ll = 0;
                  ll < loaderSettings.MAX_FLIGHTS_PER_DAY;
                  ll++
                ) {
                  ...
                  flight.scheduledDepartureTime = getDepartureTimeDaysFromDate(
                    nowAtMidnight,
                    kk
                  );
                  flight.scheduledArrivalTime = getArrivalTime(
