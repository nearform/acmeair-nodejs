...
module.exports = function(settings) {
  ...
  var nanoDBs = {};
  nanoDBs[module.dbNames.airportCodeMappingName] = nano.db.use(
    module.dbNames.airportCodeMappingName
  );
  nanoDBs[module.dbNames.bookingName] = nano.db.use(module.dbNames.bookingName);
  nanoDBs[module.dbNames.customerName] = nano.db.use(
    module.dbNames.customerName
  );
  nanoDBs[module.dbNames.customerSessionName] = nano.db.use(
    module.dbNames.customerSessionName
  );
  nanoDBs[module.dbNames.flightName] = nano.db.use(module.dbNames.flightName);
  nanoDBs[module.dbNames.flightSegmentName] = nano.db.use(
  ...
  module.update = function(
    ...
    getRevision(nanoDBs[collectionname], doc._id, function(error, revision) {
      // Has to get revision as the customer passed from ui lost revision
  ...
  module.remove = function(collectionname, condition, callback /* (error) */) {
    getRevision(nanoDBs[collectionname], condition._id, function(
      err,
      revision
    ) {
      if (!err) {
        nanoDBs[collectionname].destroy(condition._id, revision, function(
          error,
          body
        ) {
          callback(error);
  ...
  module.findBy = function(
    ...
    for (var attrName in condition) {
      if (searchCriteria.length != 0) searchCriteria += ' AND ';
      searchCriteria += attrName + ':' + JSON.stringify(condition[attrName]);
    }

    logger.debug('search:' + searchCriteria);
    nanoDBs[collectionname].search(
      'view',
      collectionname + 's',
      { q: searchCriteria, include_docs: true },
      function(err, docs) {
        if (err) {
          logger.error('Hit error:' + err);
          callback(err, null);
        } else callback(err, getDocumentFromQuery(docs));
      }
    );
  };
  ...
  function getDocumentFromQuery(document) {
    ...
    var docs = [];
    for (i = 0; i < document.total_rows; i++) docs[i] = document.rows[i].doc;
