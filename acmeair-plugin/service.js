...
class Service {
  ...
  async login(login, password) {
    const user = await this.provider.login(this.provider.dbNames.customerName, {
      _id: login,
      password: password,
    });
    if (!user) throw Boom.badData('Check your username');
    if (!user || user.password !== password)
      throw Boom.badData('Wrong credentials');
    try {
  ...
  async createSession(customerId) {
    ...
    try {
      await this.provider.insertOne(
        this.provider.dbNames.customerSessionName,
        document
      );
      return document._id;
    } catch (error) {
  ...
  async invalidateSession(sessionid) {
    ...
    await this.provider.remove(this.provider.dbNames.customerSessionName, {
      _id: sessionid,
    });
  }
  ...
  async validateSession(sessionId) {
    ...
    const now = new Date();
    try {
      const session = await this.provider.findOne(
        this.provider.dbNames.customerSessionName,
        sessionId
      );
